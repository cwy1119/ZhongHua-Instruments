...
class Musics {
    constructor() {
        var mtype = $("div.content-resizer").children("h2").text();
        var text;
        $.ajax({
            type: 'get',
            // url: '../static/json/infos.json',
            url: '/work/' + mtype,
            dataType: 'json',
            async: false,
            // data:"",
            ...
            error: function(xhr, opt) {
                alert('请求失败', mtype, opt.url, xhr.status, xhr.statusText);
            }
        })
        this.songs = text;
    }
...
class PlayerCreator {
    constructor() {
            ...
            this.loop_mode = 0; // 1 2 播放模式
            // 下方歌曲列表容器
            this.song_list = $('.music__list_content');

            // 添加 歌曲对应的背景故事文本的渲染组
            // 要求：歌曲信息要有这个标题和文本、且为html格式
            this.render_doms = { //切换歌曲时需要渲染的dom组
                title: $('.music__info--title'),
                author: $('.music__info--author'),
                image: $('.music-player__image img'),
                blur: $('.music-player__blur'),
                article_title: $("section#masterWork-page").children("article.article").children("div.article-title"), //$('.article-title'), //add
                article_content: $("section#masterWork-page").children("article.article").children('div.article-content'),
                odj_pic: $("section#masterWork-page").children("div.obj-pic") //add
            }
    ...
    init() {
        ...
        //生成播放列表
    renderSongList() {
            let _str = ''; // 根据歌曲列表生成html列表语句
            this.musics.songs.forEach((song, i) => {
                // todo 添加列表显示的信息
                _str += `<li class="music__list__item">${i + 1} ${song.mp3_name}</li>`
            });
    ...
    renderSongStyle() {
        ...
        // todo 添加歌曲信息 对应的要添加一些在html、css、js中
        this.render_doms.title.html(mp3_name);
        // this.render_doms.author.html("待续...");
        this.render_doms.image.prop('src', pic_url);
        this.render_doms.blur.css('background-image', 'url("' + pic_url + '")');
        this.render_doms.article_title.text(title); //add
        this.render_doms.article_content.html(content); //add
        this.render_doms.odj_pic.html('<img src="' + pic_url + '" />')

        //切换列表中的item的类名 play 即歌曲列表中有小喇叭的歌曲换了
    ...
    handlePlayAndPause() {
            ...
            if (this.audio.paused) { //现在是暂停的 要播放
                ...
                this.disc.image.removeClass('stop').addClass('play');
                // this.disc.pointer.addClass('play')
            } else {
                this.audio.pause();
                _o_i.addClass('icon-play').removeClass('icon-pause');
                this.disc.image.removeClass('play').addClass('stop');
